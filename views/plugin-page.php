...
<div class="ocdi  wrap  about-wrap">
	...
	<?php else : ?>
		...
		?>
			...
			<div class="ocdi__gl-item-container  wp-clearfix  js-ocdi-gl-item-container items--home">
				<?php if (array_search('home', array_column($predefined_themes, 'type')) !== false) : ?>
			...
			<div class="ocdi__gl-item-container  wp-clearfix  js-ocdi-gl-item-container items--page">
				<?php if (array_search('page', array_column($predefined_themes, 'type')) !== false) : ?>
	...
	<div class="ocdi__importing-info">
		<div class="ocdi__importing-info-container">
			...
			<button type="button" class="notice-dismiss ocdi-dissmiss"><span class="screen-reader-text">Dismiss this notice.</span></button>
		</div>
	</div>
</div>
...
function omnis_export_item_output($index, $import_file) {
	...
	$output .= '<div class="ocdi__gl-item-footer'. (! empty( $import_file['preview_url'] ) ? '  ocdi__gl-item-footer--with-preview' : '') .'">';
	$output .= '<h4 class="ocdi__gl-item-title" title="'. esc_attr( $import_file['import_file_name'] ) .'">'. esc_html( $import_file['import_file_name'] ) .'</h4>';
	$output .= '<button class="ocdi__gl-item-button  button  button-primary  js-ocdi-gl-import-data" value="'. esc_attr( $index ) .'">'. esc_html__( 'Import', 'pt-ocdi' ) .'</button>';
 	if ( ! empty( $import_file['preview_url'] ) ) {
		$output .= '<a class="ocdi__gl-item-button  button" href="'. esc_url( $import_file['preview_url'] ) .'" target="_blank">'. esc_html__( 'Preview', 'pt-ocdi' ) .'</a>';
	}
	$output .= ' </div>';
	$output .= '</div>';
